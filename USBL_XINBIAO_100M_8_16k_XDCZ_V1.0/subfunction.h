...
void Transmit_IRF_delay()
{
	...
	for(i=0;i<50;i++)
	{
		for(j=0;j<24;j++)
		{
			Input_bits[27*i+j] = (delay_sig1[i]>>(23-j)) & 0x00000001u;//移位取最低位
		}
		for(j=0;j<3;j++)
		{
			Input_bits[27*i+j+24] = (JCXBID1>>(2-j)) & 0x0001u;
		}
	}
	...
	PLL_config(&PLLconfig);				//恢复DSP时钟
	timer_low_clk(2000000,200);			//10s时延
