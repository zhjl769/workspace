...
void Produceref()
{
	...
	for(q = 0; q < 8; q++)
	{
		...
		{
			t_lfm = lfm_head/40000.0; //²ÉÑùÆµÂÊ40kHz
...
void Produceref()
{
    ...
    {
       t_lfm = (float)lfm_head/40000.0;//²ÉÑùÆµÂÊ100kHz
       Rev[lfm_head - 1] = (short)(-100*(cos(2*PI*14000*t_lfm - PI*5000*t_lfm*t_lfm/0.025)));//ÆğÊ¼ÆµÂÊ9kHz£¬½ØÖ¹ÆµÂÊ14kHz,25ms
    }
...
/*²úÉú·¢ÉäĞÅºÅ
  º¯Êı¹¦ÄÜ£º²úÉúÁ½¸öĞÅºÅ£¬Ñ¯ÎÊĞÅºÅºÍ²âÕóĞÅºÅ´æ·ÅÔÚCE2¿Õ¼ä
...
void FFTprepare(short *data, short *data_bu0, short data_N)
{
	...
	for (i = 0; i < data_N; i++)
	{
		...
		data_bu0[2*i] = data[i]>>8;//Å¼µØÖ·ÓÒÒÆ7bit
		data_bu0[2*i+1] = 0;
	}
}
			...
			else
			{
				...
				if((envelopein[i] <= Max_threshold))      //ÏòÇ°ËÑË÷£¬i»ñÈ¡ÓÒ²àµÍÓÚÏà¶ÔÃÅÏŞµÄµÚÒ»µã¡£
				{
                   ...
                   //ÕÒµ½×î´óÖµÎ»ÖÃ
					...
					//if((envelopein[Max_pos] > sum_mean) && (width > Maikuan_l) && (width < Maikuan_h) && (point_count_qian < 8) && (point_count_hou < 40)  && (SNR_HD_out > 8) )//Ïà¹Ø·åÂú×ã¾Ö²¿ĞÅÔë±ÈºÍÂö¿íÃÅ  && (SNR_HD_out > 10)
					if((envelopein[Max_pos] > sum_mean_feng_hou) && (envelopein[Max_pos] > max_ave1) && (width > Maikuan_l) && (width < Maikuan_h) && (point_count_qian < 3) && (point_count_hou < 40)   )//Ïà¹Ø·åÂú×ã¾Ö²¿ĞÅÔë±ÈºÍÂö¿íÃÅ  && (SNR_HD_out > 10)
					{
						//asm(" bclr XF ");
						pos = 1;
...
//	asm(" bset XF ");
	if (DMA_count > 1)
	{
		...
		{
			detect(Rev_envelope,2448,&Pos_flag,&endpos);//¼ì²âº¯Êı×î´óËÑË÷100¸öÏà¹Ø·å£¬¼ì²âÊ±¼ä·¶Î§Îª0.12ms~2ms
		}
		point_count_zong = 0;
...
void take_action1()
{
	...
	long temp;
	//¼ÆËã×ª·¢ÑÓ³Ù
	resend_delay = Time_Delay - 51680 - (Uint32)((2*NFFT - endpos_float)*25);	//µ¥Î»¶¼ÊÇ1us,ÈıµãÄÚ²å
...
// Ñ¹Á¦µ÷ÖÆ200ms£»
	...
	SD_write[34] = 0;                         //ÎŞÖ¸î
	SD_write[37] = (NOISE_TH_NEW) & 0x0ff;    //Ïà¹Ø·åµÄ»¬¶¯¾ø¶ÔÃÅÏŞ	µÍ8Î»
...
void take_action2()
{
	...
	resend_delay = 25000000;//Çı¶¯ÊÍ·ÅÊ±¼ä:25s,ÊÍ·ÅÆ÷ÊÍ·ÅÊ±¼äĞè24s¡£
    //resend_delay = 100000;
	timer1_cnt = 0;
 	IFR1 = 0x0ffff;
 	IER1 |= 0x0040;
...
void take_action3()
{
	...
	if(resend_delay > 0)
	{
		...
		timer1_cnt = 0;
	 	IFR1 = 0x0ffff;
	 	IER1 |= 0x0040;//¿ªÆôDMA1ÓÃÓÚ·¢ÉäÑ¯ÎÊĞÅºÅ
		TIMER_start(mhTimer1);//´ò¿ª¶¨Ê±Æ÷1Ê±
		while(1)
		{
			if(timer1_cnt == resend_delay)
			{
				TIMER_stop(mhTimer1);//¹ØÕ¶¨Ê±Æ÷1
				IFR1 = 0x0ffff;
	 			IER1 &= 0x0ffbf;//¿ªÆôDMA1ÓÃÓÚ·¢ÉäÑ¯ÎÊĞÅºÅ
				break;
			}
...
void FSKdetect()
{
	...
	if (block2 == 6) //FFT¼ì²âÆµÂÊ
	{
		...
		//²é±í²âÆµ
		if ((f_code >= 82.5-1.5)&&(f_code <= 82.5+1.5))
		{
		  testpinlv_flag = 1;	 
		}
		else
...
void detect_signal()
{
	while((Flag_answer==0) && (FLAG_RS232_REVOK == 0))//Ã»ÓĞ¼ì²âµ½ĞÅºÅ»òÕß´®¿ÚÃüÁî
	{
		if (DMAdone == TRUE)
		{
			...
			{
				//asm(" bset XF ");
				memcpy(Rev_save,Rev,2*NFFT);//±£ÁôÉÏÒ»´ÎÊı¾İ£¬¼ÆËãÔëÉù
				//asm(" bclr XF ");
			}
			if(DMA_count < 2)
			{
