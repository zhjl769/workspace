...
DMA_Config  Send_config = {
		...
		(DMA_AdrPtr)&Send_sig,                /* DMACSSAL */
		0,                     				 /* DMACSSAU */
		(DMA_AdrPtr)(GPIO_ADDR(IODATA)),
		0,                                     /* DMACDSAU */
		Time_Sig*1000,
		1,                                     /* DMACFN   */
		0,                                     /* DMACFI   */
...
void DMA_init(void)
{
	...
	sendsrc0AddrHi = (Uint16)(((Uint32)(Send_config.dmacssal)) >> 15) & 0xFFFFu;//DMA0发射源地址高字节
	sendsrc0AddrLo = (Uint16)(((Uint32)(Send_config.dmacssal)) << 1) & 0xFFFFu;//DMA0发射源地址低字节
	senddst0AddrHi = (Uint16)(((Uint32)(Send_config.dmacdsal)) >> 15) & 0xFFFFu;//DMA0发射目的地址高字节
	senddst0AddrLo = (Uint16)(((Uint32)(Send_config.dmacdsal)) << 1) & 0xFFFFu;//DMA0发射目的地址低字节

	Send_config.dmacssal = (DMA_AdrPtr)sendsrc0AddrLo;//发射源地址EMIF
	Send_config.dmacssau = sendsrc0AddrHi;
	Send_config.dmacdsal = (DMA_AdrPtr)senddst0AddrLo;//发射目的地址GPIO
	Send_config.dmacdsau = senddst0AddrHi;
	...
	DMA_config(hDmaSend0,&Send_config);
...
/*McBSP初始化配置  函数功能：初始化配置AD采样率为100kHz  */
...
void McBSP_init()
{
	...
	SRGR2_0 = 0x3027;
	//CLKSM=1，McBSP internal input clock；0x3063-FPER=99,0x3031-49,0xc7-199,帧同步信号周期为20;FPER=99;/FPER决定CLKR与fs之间的比例系数。CLKin=100M,CLKG=CLKin/24=4M,fs=CLKG/(fper+1)=100M
	SRGR1_0 = 0x0218;
	//帧同步脉冲3个CLKG宽度;时钟分频次数为//决定FWID,CLKGDV,0x09-9,0x63-99,0x18_25  fs=100k,clk=100MHz,那么clkR为clk/25(0x18=24)=4M//原始数据0x218
...
void System_initial()
{
	...
	CLKMD = 0x2cb2;// clk = 8M; mult=25;div=2;25*8M/2=100M. 0x2cb2-100MHz. 0x2C92-200MHz
...
void uart_config()
{
	...
	SD_write[25] = 0x0ff;//校验位
	SD_write[26] = SD_status;//SD卡状态，1为正常，0为不正常
	SD_write[27] = 0x0ff;//校验位
	I2C_Write(0xd700, SD_write, 28, 0x0050);  //分页写入SD_write前28位的数据
}
