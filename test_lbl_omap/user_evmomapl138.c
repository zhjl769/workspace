...
//	for(i=0;i<20;i++){};					//600clock
	...
	SETBIT(MCBSP1->SPCR, 0x00010000);	//6
...
void evm_init_edma_setup(CSL_Edma3ccRegs *edma3ccRegs)
{
	...
	//建立EDMA时间过程：
	//向第一ECR寄存器写0xffffffff，硎厩宄械腅DMA事件
...
void evm_init_gpio_extern()
{
	...
	*(unsigned volatile int *)GPIO_BINTEN = 0x00000040;//enable BANK6 Interrupe

	GPIO_setDir(GPIO_BANK0, GPIO_PIN13, GPIO_OUTPUT);		
	GPIO_setOutput(GPIO_BANK0, GPIO_PIN13, OUTPUT_LOW);	//增益54dB(0)  74dB(1) 

	GPIO_BANK23->DIR = 0x00000000;
	GPIO_BANK45->DIR = 0x00000000;
	GPIO_BANK67->DIR = 0x00000040;		//CPLD2DSP GPIO6[6]
	GPIO_BANK67->SET_RIS_TRIG = 0x00000040;
	GPIO_BANK67->CLR_FAL_TRIG = 0x00000040;
	...
	GPIO_setOutput(GPIO_BANK5,  TRANS_SYNC, OUTPUT_LOW);//设置GPIO输出低电平
}
...
void transmit(uint16_t *signal, int n)
{
	...
	SETBIT(GPIO_BANK45->OUT_DATA, (0x10000000));;//设置GPIO输出高电平
	for( i =0; i < n; i++)
	{
		temp1 = signal[i]; 
		for(j = 0; j<16;j++)
		{
		   ...
		   {
			  while(!Trans_flag);
			  SETBIT(GPIO_BANK45->OUT_DATA, (0x02000000));
			  CLRBIT(GPIO_BANK45->OUT_DATA, (0x00400000));
			  Trans_flag = 0;
		   }
		   else
		   {
...
void DSP_interrupt_ARM()
{
	...
	SYSCONFIG->CHIPSIG_CLR = 0x00000008;
