...
//*************************initial program***********************************//
	...
	init_global_var(); 	
	gen_sig(Ref_fft1, Sig_flag[0], &CHIP_L, Refsig_len, F, RAMSES_CODE, w, Temp_buffer1, Temp_buffer2);
	...
	gen_transmit_sig(Trans_signal, &Trans_signal_len, Trans_signal_flag, F, RAMSES_CODE);
		...
		{				  
	        ...
	        while(Block_num <= Max_Buffer)
	        {
		       	...
		       	{        
					...
					memcpy(ddr2_buffer+Block_num*IN_LENGTH, Fifo1+2*IN_LENGTH, IN_LENGTH*2);
					Block_num++;

					get_fft(Sig_fft, Fifo1+IN_LENGTH, NFFT, w, NFFT, Temp_buffer1);		

					if(Sig1.flag == 0)
					{
						get_xcorr(Correlation1, Sig_fft, Ref_fft1, NFFT, w, Temp_buffer1, Temp_buffer2);
						get_envelop(Envelop1, Correlation1, Coef_fft, NFFT, w, Temp_buffer1, Temp_buffer2);
						detection(&Sig1, Envelop1, &Parameter_sig, Block_num, Close_num);	
						if(Sig1.flag == 1)
					...
					if(Sig2.flag == 0)
					{
						...
						get_envelop(Envelop2, Correlation2, Coef_fft, NFFT, w, Temp_buffer1, Temp_buffer2);
					  	detection(&Sig2, Envelop2, &Parameter_sig, Block_num, Close_num);
						if(Sig2.flag == 1)
						{
					...
					{						
						...
						detection(&Sig3, Envelop3, &Parameter_sig, Block_num, Close_num);
						if(Sig3.flag == 1)
					...
					if(Sig4.flag == 0)
					{
						...
						detection(&Sig4, Envelop4, &Parameter_sig, Block_num, Close_num);
						if(Sig4.flag == 1)
