...
void gen_sig(short *ref_fft, int sig_flag, short *chip_l, int refsig_len, double *f, double (*code)[8], int *w, int *temp_buffer1, int *temp_buffer2)
{
	...
	if(sig_flag >= 0 && sig_flag <= 23)
	{
		for(j=0;j<8;j++)
		{
			...
			{
				*(temp_buffer1+2*(refsig_len-k-1)) = (int)(32767*sin(2*PI*f0*(k-m)/fs+pha));   //偶数地址为实部，奇数地址存虚部，实部为实际信号，虚部为0
			}
...
{									
		...
		{		
			...
			Parameter_sig.snr_abso = *(int *)&ddr2_cmd[arm_parameter_abs_snr_threshold];      //绝对信噪比
			Parameter_sig.snr_rela = *(int *)&ddr2_cmd[arm_parameter_rel_snr_threshold];         //向前虚警信噪比
			Parameter_sig.snr_rela2 = *(int *)&ddr2_cmd[arm_parameter_rel2_snr_threshold];     //向后混响信噪比
			Parameter_sig.Maikuan_l = *(int *)&ddr2_cmd[arm_parameter_min_pulse_width];      //峰宽下限
			Parameter_sig.Maikuan_h = *(int *)&ddr2_cmd[arm_parameter_max_pulse_width];     //峰宽上限
...
void get_xcorr(int *correlation, int *sig_fft, short *ref_fft, int n, int *w, int *temp_1, int *temp_2)
{
	...
	for(j=0;j<n;j++)
	{
		temp_1[2*j]   = (sig_fft[2*j]*ref_fft[2*j]-sig_fft[2*j+1]*ref_fft[2*j+1])>>9;               //实部
		temp_1[2*j+1] = (sig_fft[2*j]*ref_fft[2*j+1]+sig_fft[2*j+1]*ref_fft[2*j])>>9;            //虚部
	}
...
void detection(struct Sig *sig, int *envelop, struct Para *parameter, int block_num, int close_num,short *fifo1)
{
	short sig_max = 0;
	...
	{
		sig->th_abso = (int)(sig->th_abso*(1 - 1/noise_l)) + mean(&envelop[3*IN_LENGTH], IN_LENGTH, 2)/noise_l;
       
		sig->th_max[2] = sig->th_max[1];
		sig->th_max[1] = sig->th_max[0];
		sig->th_num[2] = sig->th_num[1];
		sig->th_num[1] = sig->th_num[0];
		max(&sig->th_max[0], &sig->th_num[0], envelop+3*IN_LENGTH, IN_LENGTH, 1);      //取1个buffer最大值
		...
		weifeng_num = find_num(envelop+IN_LENGTH, 3*IN_LENGTH, th_max1, weifeng_ind); //搜索区间内所有可能峰值的数量      最大值前后10ms内过动态门限点的个数
					...
					else if(envelop[k] <= th_after)                                                        //找到一个后端
					{
						for(m=max_num; m>max_num-50; m--)                                    //峰值点向前50点找前端
						{
							if(envelop[m] <= th_before)                                               //找到前端
							{
								...
								ave[0] = mean(envelop+m-offset-1*l, l, 2)*parameter->snr_rela/2;	            //前1ms的距离搜索区域信噪比      调频信号相关混叠
								ave[1] = mean(envelop+m-offset-2*l, l, 2)*parameter->snr_rela/2;
								ave[2] = mean(envelop+m-offset-3*l, l, 2)*parameter->snr_rela/2;
								ave[3] = mean(envelop+m-offset-4*l, l, 2)*parameter->snr_rela/2;
								ave[4] = mean(envelop+m-offset-5*l, l, 2)*parameter->snr_rela/2;
								ave[5] = mean(envelop+m-offset-6*l, l, 2)*parameter->snr_rela/2;
								ave[6] = mean(envelop+m-offset-7*l, l, 2)*parameter->snr_rela/2;
								ave[7] = mean(envelop+m-offset-8*l, l, 2)*parameter->snr_rela/2;
								max(&max_ave1, &ave_num, ave, 8, 1);                                  //向前取区域信噪比
								max_ave2 = mean(envelop+k+l, l*3, 2)*parameter->snr_rela2; //从主峰第一个零点向后1ms混响区域信噪比
								weifeng_before = find_num(envelop+m-offset-10*l, 10*l, max_val*1/2, weifeng_ind);   //向前找超过峰值1/2点 向前10ms
								weifeng_before *= weifeng_ind;
								weifeng_after  = find_num(envelop+k+l, 10*l, max_val*1/2, weifeng_ind);                     //向后找超过峰值1/2点   向后5ms？？？
								...
								{
								    max_short(&sig_max, &sig_max_location, &fifo1[max_num-64-400-512], 400, 1);
                                    sig_max_num  = find_num_short(&fifo1[max_num-64-400-512], 400, sig_max/2, 1);    //超过一半峰值的点数
									if(sig_max_num < 20) //取出尖刺噪声
										break;  
									sig->snr = parameter->snr_rela*max_val/max_ave1;//显示所有计算的参数  缺前后信噪比
									sig->flag = 1;
