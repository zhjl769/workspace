...
#define PINMUX_EMIFA_NAND_MASK_0       (0xFFFFFFFF)
#define PINMUX_EMIFA_NAND_VAL_0        (0x11118811)
...
#define TIMEOUT_NAND                   (2000) // in us.

// nand flash
#define NAND_FLASH_BASE                (0x62000000)                     //EMIFA  CS3
...
// expected device id for Micron K9K8G08U0A.
#define EXPECTED_MFG_ID                (0xEC)
#define EXPECTED_DEV_ID                (0xD3)
...
uint32_t NAND_init(void)
{
  ...
  // step1:开启EMIFA模块的时钟使能
   EVMOMAPL138_lpscTransition(PSC0, DOMAIN0, LPSC_EMIFA, PSC_ENABLE);
   // step2:配置引脚复用寄存器，主要是配置几个控制信号：数据线D0~D7,地址线：A1/A2,控制线：EMA_OE/EMA_WE/EMA_CS3/EMA_CS4
   ...
   // config nand fcr reg.
   // step3:配置NANDFCR寄存器，主要进行ECC校验的设置
   CLRBIT(EMIFA->NANDFCR, ECCSEL_MASK);
   SETBIT(EMIFA->NANDFCR, ECCSEL_CS3);
   SETBIT(EMIFA->NANDFCR, CS3NAND);

   // send reset cmd to nand.
   // step4:发送复位命令给NAND，首先将CLE:ALE置位为H:L模式即写入0x10，表示写命令
   //	  （加上0x62000000基地址是为了操作CS3等控制信号）
   //       然后在数据线上写入：0xff，表示复位命令 
...
uint32_t readSpareData(uint32_t in_block, uint32_t in_page, uint8_t *spare_data)
{
   ...
   // step1:写入命令，使能读操作
   writeLatchEnable(NAND_OFFSET_CLE, NAND_CMD_READ);
   // step2:写入block的Spare Field地址
   writeAddressBytes(ADDR_CYCLES_5, in_block, in_page);
   // step3:写入0x30地址命令，见文档41页
   writeLatchEnable(NAND_OFFSET_CLE, NAND_CMD_READ_CONFIRM);
   rtn = waitFlashReady(TIMEOUT_NAND);
   if (rtn != ERR_NO_ERROR)
      return (rtn);
...
{
   //写地址字节包含5个clock:1st写A0~A7;2nd写A8~A11;3rd写A12~A19;4th写A20~A27;5th写A28~A30;
   //flash映射的每个page包含（2048+64）个字节，最大的寻址基地址低位有12根地址线。
   //读取有效的是后面64个字节，但是后面的程序把之前的数据全部读取了，将整个block块的数据全部读取
   //因此前两个周期写入的是0x0000
   ...
   //第三个地址写入page值，每个块有64个page，因此写入page的低6位值，结合block的低位值
   //1G字节的存储器可以选择的地址线有30根，其中（低12根列地址加4bit0）组成CA，64页由6根地址线寻址，外加2根block寻址组成Addr3
   //然后block移位2bit，组成Addr4，然后右移10bit，取低两位组成Addr5
...
void readData(uint8_t *dest_buffer, uint32_t in_length)
{
   ...
   //选择地址偏移位NAND_OFFSET_DATA=0x00，而且进行读操作，表示数据输出命令
...
uint32_t verifyDeviceId(uint32_t in_timeout)
{
   ...
   // 读/验证设备ID号的过程
   // step1:首先将CLE:ALE置位为H:L模式，即写入0x10，表示写命令
   //       然后进行写入读设备ID号的命令，即写入0x90
   writeLatchEnable(NAND_OFFSET_CLE, NAND_CMD_READ_ID);
   // step2:将CLE:ALE置位为L:H模式，即写入0x08，表示接下来写入的地址信息
