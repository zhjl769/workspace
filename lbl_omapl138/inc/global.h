...
#pragma DATA_SECTION(ddr2_beacon_detection_flag, "ddr2_dat")		//信号检测到后 上传检测标志
char ddr2_beacon_detection_flag[4];
#pragma DATA_SECTION(ddr2_beacon_detection_flag_num, "ddr2_dat")	//第几个信号检测到	
char ddr2_beacon_detection_flag_num[4];
...
//全局变量参数
					      //s_m,s_a,s_r,s_r2,M_l,M_h,w_all,w_b,w_a,reverb,th
...
char Work_mode = 0; 	//应答模式1 同步模式0
char sys_gain = 0;		//系统增益标志

int Max_Buffer= 149;	//根据检测周期调整 
...
double F[8] = {9640, 10540, 11250, 12030, 12850, 13600, 14350, 15150};        //跳频频段
short CHIP_L = 50;                             //码片长度为fs*L/8=40*10/8=50              //每个频率的采样点数
int Refsig_len = 400;                                                                                      //总采样点数
int Sig_flag[4] = {6,14,10,20};                                                                       //信号编码    matlab中是[7 15 11 21]
//发射部分
#pragma DATA_SECTION(Trans_signal, "buffer_send_sig")
unsigned short Trans_signal[2500]; 
int Trans_signal_flag = 0;
int Trans_signal_len = 1250;

double RAMSES_CODE[24][8] = {                                                             //DSP中是0-7，MATLAB中是1-8
7, 2, 3, 0, 6, 5, 1, 4,
0, 2, 5, 1, 6, 7, 4, 3,
0, 3, 4, 2, 6, 5, 7, 1,
0, 4, 2, 7, 6, 3, 5, 1,
