...
#define CS_LOW()    P5OUT &= ~BIT6              // Card Select
#define CS_HIGH()   SPI_TX_DONE; P5OUT |= BIT6  // Card Deselect
...
#define SPI_TX_DONE     while(UCB0STAT & UCBUSY)
...
char mmcSetBlockLength (const uint32);
...
char mmcReadRegister(const char, const uint8, uint8 *pBuffer);
