...
void Produce_long_LFM()//15-9k,1.62s,LFM
{
	volatile unsigned long lfm_head;
	volatile double t_lfm;
	volatile short templfm;
	...
	for(lfm_head = 0; lfm_head < 1555200; lfm_head++)
	{
		...
		templfm = (short)(20000*(sin(2*PI*15000*t_lfm - PI*6000*t_lfm*t_lfm/1.62)));
...
//----------------------------数据转换---------------------------------------------
/*	delay_sig1[50]中存储了50组时延信息，每个时延信息用4字节（32bit）表示（因为5509A中存储的数据必须为16bit的整倍数）
	然而24bit已经足够表示时延信息，2^24=16777216us，约等于16.7s，乘以声速1500再除以2得到24bit时延能表示的最大单程传播距离，约为12.5km
 	 下面的程序将 delay_sig1[50]中的数据依次处理成27bit数据（24bit表示时延，3bit表示信标号），但是每个比特用一个char类型表示
	最后形成1350个char准备发送（每个char都是0或1）
 */

void Input_bits_change(unsigned long int *delay_sig , int ID_code)
{
	int i,j;
	for(i=0;i<50;i++)
	{
		for(j=0;j<24;j++)
		{
			Input_bits[27*i+j] = (delay_sig[i]>>(23-j)) & 0x00000001u;//移位取最低位
		}
		for(j=0;j<3;j++)
		{
			Input_bits[27*i+j+24] = (ID_code>>(2-j)) & 0x0001u;
		}
	}
}

void Send_LFM_CW_Tx(short count , short delay)
{
	short delay_temp;
	while(count > 0)
	{
		delay_temp = delay;
		Send_Tx_prepare(Pwm_result);		//发送通信信号的准备工作
		Send_pre_sig(Sig_LFM , 38400 ,1);	//9-16k.40ms,LFM
		timer_low_clk(150000,192);			//150ms时延
		Send_pre_sig(Sig_CW , 48000 , 2);	//100ms,9k,CW
		timer_low_clk(150000,192);			//150ms时延
		Send_Tx(Pwm_result);				//发送通信信号
		while(delay_temp > 0)
		{
			timer_low_clk(1000000,192);		//1s时延
