...
void Read_const_I2C()
{
	Uint32 i, j ;
	Uint32 ii = 0;				//待读出数据计数
	Uint32 addr = 0;   			//从IIC设备的起始数据地址
	Uint8 Rtmp[Page_Length];	//从IIC设备读取数据的缓存空间
	/*常数表预先清零*/
	for(i=0;i<12416;i++)
	{
		Pwm_List[i] = 0;
	...
	/*开始读取*/
	for(i=0;i<97;i++)//正好97块数据，每块数据128字节
	{
		I2C_Read(addr, Rtmp, sizeof(Rtmp), EEPROM_addr); //读出写入的128Bytes的数据
		for (j=0; j < sizeof(Rtmp); j++)
		{
			Pwm_List[ii] = Rtmp[j];					//Pwm_List中的数据搬移到临时空间
			ii = ii+1;
		}
	}
}
