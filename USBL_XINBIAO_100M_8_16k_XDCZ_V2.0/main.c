...
void main()
{
	...
	//-------------------------------------I2C读取常数表--------------------------
	//PLL_config(&PLLconfig_I2C);//切换至48MHz时钟，用于I2C总线通信
	/* CSL的I2C SETUP函数有问题，不推荐使用，包括写入和读取函数，需要将I2CSTR寄存器中的BB位置1*/
	I2C_FSET(I2CSTR,BB,0x1);
	I2C_config(&I2Cconfig);
	I2C_FSET(I2CMDR,  IRS,0);//IRS置零，准备配置I2C预分频寄存器
	I2C_RSET(I2CPSC,  0x0013u);//配置I2C预分频寄存器，20分频，5MHz
	I2C_RSET(I2CSTR,  0xFFFFu);//由于IRS置零会复位I2CSTR，所以此处重新配置I2CSTR
	I2C_RSET(I2CCLKL, 0x0014u);//20
	I2C_RSET(I2CCLKH, 0x0014u);//20
	I2C_FSET(I2CMDR,  IRS,1);//时钟配置完毕，IRS置1

	I2C_RSET(I2COAR,  0x0000);  //主IIC模块自身的地址
	Read_const_I2C();
	//PLL_config(&PLLconfig);	//恢复DSP时钟，用与定位程序，100MHz
	//--------------------------------------------------------------------------
	Senddata0();
