...
*///bywyh EMIF pdf clk=144MHz
	CE0_1=0x112F;//16bit步存储器;读建立时间5ns;读选通时间75ns;读保持时间15ns
...
void Produceref()
{
    ...
    for(lfm_head = 1; lfm_head < 1001; lfm_head++)
    {
       ...
       Rev[lfm_head - 1] = (short)(-100*(cos(2*PI*20000*t_lfm + PI*10000*t_lfm*t_lfm/0.020)));//起始频率9kHz，截止频率14kHz,25ms
...
void Producelfm()
{
   ...
   for(lfm_head = 0; lfm_head < 20000; lfm_head++)
   {
      t_lfm = (float)lfm_head/1000000.0;//采样频率1MHz
      templfm = (short)(20000*(cos(2*PI*20000*t_lfm + PI*10000*t_lfm*t_lfm/0.02)));//起始频率22kHz，截止频率30kHz,25ms
	  if(templfm < 0)
	  {
...
void FFTprepare_FSK(short *data, short *data_bu0, short data_N)
{
	...
	for (i = 0; i < data_N; i++)
	{
		...
		data_bu0[2*i] = data[i]>>12;//嫉酚乙�bit
		data_bu0[2*i+1] = 0;
	}
}
...
void detect_signal()
{
	...
	while((Flag_answer==0) && (FLAG_RS232_REVOK == 0) && ((IODATA & 0x0010) != 16))//没有检测到信号或者串口命令或则是外部电源存在
	{
