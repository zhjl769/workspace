...
/*************************************************
  ...
  Author:  Han     Version:   1.4     Date: 2012.6.30
  Description:   检测信号为25ms长LFM信号，9-14kHz，下调频
  				 AD驱动采用AD7663，挂载在McBSP0上，采样率100kHz
...
{
	short i;
    //GPIO初始化，将GPIO7、4作为输入，其他作为输出
	IODOR = 0x006f;
	IODATA = 0x0040;   //GPIO6拉低
	//变量初始化
	variable_init();
	//禁止全局中断
	asm(" bset intm ");
	//CSL初始化
	CSL_init();	
	//系统硬件初始化，包括系统主频144MHz，SDRAM初始化，TL16C752初始化
	System_initial();
	...
	//产生发射信号
	Producelfm();//产生发射信号		
	//McBSP初始化，主要完成A/D驱动控制，采样率设置为100kHz
	McBSP_init();
	//内部AD初始化，采集电池电压
	InitADC();
	//DMA初始化
...
//	Flag_answer = 1;
    Time_Delay = 100000;//75ms对应75000us，定时精确到us
	NOISE_TH_NEW = 1000;
	NOISE_TH_HD = 0;
