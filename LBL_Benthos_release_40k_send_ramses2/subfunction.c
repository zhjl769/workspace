...
void Produceref()
{
	...
	memset(Rev,0,sizeof(short)*NFFT);
    //定位信号20ms
    for(lfm_head = 1; lfm_head < 801; lfm_head++)
    {
       t_lfm = (float)lfm_head/80000.0;//采样频率100kHz
       Rev[lfm_head - 1] = (short)(-100*(cos(2*PI*22000*t_lfm + PI*8000*t_lfm*t_lfm/0.010)));//起始频率22kHz，截止频率30kHz,20ms
    }
