...
void Produceref()
{
    ...
    //定位信号10ms
    for(lfm_head = 0; lfm_head <1000; lfm_head++)
    {
       t_lfm = (float)lfm_head/100000.0;//采样频率100kHz
       Rev[lfm_head] = (short)(-100*(cos(2*PI*22000*t_lfm + PI*8000*t_lfm*t_lfm/0.010)));//起始频率30kHz，截止频率22kHz,10ms
...
void Producelfm()
{
   ...
   for(lfm_head = 0; lfm_head < 10000; lfm_head++)
   {
      ...
      templfm = (short)(20000*(cos(2*PI*22000*t_lfm + PI*8000*t_lfm*t_lfm/0.01)));//起始频率30kHz，截止频率22kHz,10ms
	  if(templfm < 0)
	  {
...
void Revprepare()
{
	...
	for (i = BUF_LEN_MAX; i < NFFT; i++)//BUF_LEN_MAX=NFFT-Rev_resave=3096
	{
...
void FFTprepare(short *data, short *data_bu0, short data_N)
{
	...
	for (i = 0; i < data_N; i++)
	{
		...
		data_bu0[2*i] = data[i]>>7;//偶地址右移7bit,,把数据的类型为short转换为int型，//同时除以256完成FFT计算之前归浚浚?
		data_bu0[2*i+1] = 0;
	}
}
...
//	long  sum = 0;
	...
	//判断绝对门限 开始搜索检测相关峰起点
	for(i = 1308-200; i < len+100; i++)///向前面的点进行寻找,len为4404
	{
		if(envelopein[i] > NOISE_TH_NEW)
		{
			Flag_sig = 1;
			startpos = i;//检测到了，Flag_sig置1,将位置赋给startpos（大于滑动门限的点的值）
			break;
		}
	}
	...
	if(Flag_sig)  
	{
		...
		for(i = startpos;i < len+100; i++)//从开始到最后100点搜索最大值
		{			
			if(flagg == 0)
			{
				if(envelopein[i] >= NOISE_TH_NEW)
				{
					...
					pos_begin = i;//大于峰值门限的点赋值为pos_begin
					Max_val = 0;
					for(j = i;j < i + Maikuan_h; j++)//此处修改为脉宽上限值38，理论脉宽的3倍
					{
						if(Max_val < envelopein[j])
						{
							Max_val = envelopein[j];
						}
					}                      //找到相关峰中的最大值赋值为 Max_val
					Max_threshold = (short)Max_val*0.5;//Max_threshold为相关峰中的最大值的一半
			...
			else//找到第一个大于峰值门限了、向前搜索，i获取左侧低于相对门限的第一点。
			{
				...
				if((envelopein[i] <= Max_threshold))      //Max_threshold为最大峰值的一半的值
				{
					...
					for(j = pos_begin;j >= startpos; j--) //pos_begin为左侧低于相对门限的第一点。
					{
						...
						{
							pos_begin = j;                //把低于最大峰值一半的值的点找到。并更新到pos_begin
							break;
						}
					}
                   ...
                   //找到最大值位置
					...
					//计算峰前信噪比
                    ave[0] = mean_zh(&envelopein[Max_pos - 107], 100, 2)<<2;//乘以4
					ave[1] = mean_zh(&envelopein[Max_pos - 207], 100, 2)<<2;
                    ave[2] = mean_zh(&envelopein[Max_pos - 307], 100, 2)<<2;
					ave[3] = mean_zh(&envelopein[Max_pos - 407], 100, 2)<<2;
                    ave[4] = mean_zh(&envelopein[Max_pos - 507], 100, 2)<<2;
					ave[5] = mean_zh(&envelopein[Max_pos - 607], 100, 2)<<2;
                    ave[6] = mean_zh(&envelopein[Max_pos - 707], 100, 2)<<2;
					ave[7] = mean_zh(&envelopein[Max_pos - 807], 100, 2)<<2;
					...
					sum_mean_feng_hou = (sum_feng_hou>>9); //局部信噪比的2倍后取平均值，，向右移动9位，即除以512
					sum_feng_hou = (sum_mean_feng_hou*2);//局部信噪比为大于均值的2，2为待定参数，*2
                    ...
                    //加入多峰判决减少虚警
					point_count_qian = 0;
					for(j = Max_pos-510; j < Max_pos-10; j++)//峰500个点即5ms内判断是否存在多峰
					{
					   if (envelopein[j] > half_envelope_max)
					   {
					...
					point_count_hou = 0;
					for(j=Max_pos+(Theory_wide>>1);j<Max_pos+507;j++)//峰后500个点即5ms内判断是否存在多峰
					{
					   if (envelopein[j] > half_envelope_max)
					   {
					...
					if((envelopein[Max_pos] > sum_mean_feng_hou) && (envelopein[Max_pos] > max_ave1) && (width > Maikuan_l) && (width < Maikuan_h) && (point_count_qian < 3) && (point_count_hou < 130)   )//相关峰满足局部信噪比和脉宽门  && (SNR_HD_out > 10)，65为3倍的理论脉宽
					{
						//asm(" bclr XF ");
						pos = 1;
			...
			//			neicha(envelopein[Max_pos-2], envelopein[Max_pos], envelopein[Max_pos+2], Max_pos, Max_pos_float);
...
//	memset(Rev_xcorr+4076,0,20);
	j = 0;
	for(i = BUF_LEN_MAX; i  < NFFT; i++) //BUF_LEN_MAX-接收缓存区长度,为NFFT-本地信号长度=3096
	{
		Rev_xcorrout[j] = Rev_xcorrout[i];
		j++;		
	}//将相关结果重叠保留

	for(i = Rev_resave; i  < NFFT; i++)   //i = Rev_resave=1000
	{		 
	...
	j = 0;
    for(i = 3096; i < 7500; i++)//3096个点开始，挪出3096个点的空间
	{
	    Rev_envelope[j] = Rev_envelope[i];  
		j++;
	}
	//完成数据更新
	j = 4404;//从4404点开始更新，总更新点数为3096
	for(i = Rev_resave; i < NFFT; i++)  //i = Rev_resave，为本地信号长度，1000
	{
    ...
    sum = sum>>12;
	NOISE_TH_huadong = NOISE_TH_huadong*0.98 + (float)(sum*0.05);
...
******************************************************/
	...
	point_count_zong = 0;
	for(j=1308;j<4404;j=j+7)   //j=1308，检测上次的相关结果，共3096个点，步进值为7，理论脉宽的一半，
	{
...
//	asm(" bset XF ");//XF管脚置高
	...
	if (DMA_count > 1)
	{
		if(point_count_zong <12) //最多允许存在6个相关峰
		{
			detect(Rev_envelope,4404,&Pos_flag,&endpos);//检测函数最大搜索100个相关峰，检测时间范围为0.056ms~2.85ms  BUF_LEN_MAX
		}
		point_count_zong = 0;
...
void take_action1()
{
	...
	long temp;
	//计算转发延迟
	resend_delay = Time_Delay - 41388 - (float)((float)((7500.0 - endpos_float))*10);	//单位都是1us,三点内插41374
//	resend_delay = Time_Delay - 41380 - (Uint32)((float)((7500 - endpos))*10);       //单位都是1us,无内插
...
// 压力调制200ms；
	...
	if(FLAG_yl == 0)
	{
		resend_delay =150000 - 41380 - (Uint32)(0.134*timer1_cnt) + (Uint32)(depth*200);//单位1us;准确数据。
	}
	else if(FLAG_yl == 1)
	{
		resend_delay = 150000 - 41380 - (Uint32)(0.134*timer1_cnt) + (Uint32)(depth*200);//单位1us
	}
...
void take_action3()
{
	...
	noise_10ms = (short)(temp>>9);
	uart_config();
	SD_write[28] = (Rev_envelope[endpos]) & 0x0ff;//相关峰最大值低8?
	SD_write[29] = (Rev_envelope[endpos]>>8) & 0x0ff;//相关峰最大值高8位
			...
			if(resend_delay > 0)
			{
				...
				while(1)
				{
					if(timer1_cnt == resend_delay)
					{
						TIMER_stop(mhTimer1);//关闭定时器1
						IFR1 = 0x0ffff;
			 			IER1 &= 0x0ffbb;//开启DMA1用于发射询问信号,保持外部中断0停止
						break;
					}
				}
			}
