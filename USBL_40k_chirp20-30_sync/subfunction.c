...
void Produceref()
{
    ...
    for(lfm_head = 1; lfm_head < 401; lfm_head++)
    {
       ...
       Rev[lfm_head - 1] = (short)(-100*(cos(2*PI*8000*t_lfm - PI*10000*t_lfm*t_lfm/0.01)));//起始频率20kHz，截止频率30kHz,10ms
    }
...
void Producelfm()
{
   ...
   for(lfm_head = 0; lfm_head < 10000; lfm_head++)
   {
     ...
     // templfm = (short)(20000*(cos(2*PI*20000*t_lfm + PI*10000*t_lfm*t_lfm/0.01)));//起始频率20kHz，截止频率30kHz,10ms
      templfm = (20000*(cos(2*PI*9000*t_lfm + PI*7000*t_lfm*t_lfm/0.01)));//起始频率20kHz，截止频率30kHz,10ms
	  if(templfm < 0)
	  {
